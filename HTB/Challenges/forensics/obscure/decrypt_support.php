<?php
$k = "80e32263";
$kh = "6f8af44abea0";
$kf = "351039f4a7b5";
$p = "0UlYyJHG87EJqEz6";


// First POST


/*

$request = "3Qve>.IXeOLC>[D&6f8af44abea0QKxo+HM4thMoMKWcSng9UZNbdc4WFhO2jaU4eMhPaDTePEuB48JstWIb4aEirLpXpdgb7g8Bx/IGI/JLbVRcFack+r90YxXpmBA1wQKaU9jeRhvp7imF351039f4a7b5+'Qn/?>-
e=ZU mx";

$response = "0UlYyJHG87EJqEz66f8af44abea0QKzo43k49AMoNoVOfAMh+6h3euEZJvkTlblqP34rlZqPhxDgKLYMz7NpqfQ9IR9FOXy0OfVbUgo/PF3MxrMw/JOdJebwjE2y6VAxUFnyA4H4dHQNgV49YatbqT0it9IXYf5kzoE4+kfGnZ/dTAsyCesTC0i5V+gJQw6bYm/nU3U/lrYGyl+dgvIOURfl0fvGm0hmr0RZKQ==351039f4a7b5";


*/


// Second POST

/*

$request = "3Qve>.IXeOLC>[D&6f8af44abea0QKxI+Ak49hMoNaXoypsATiJfd3clJ+KmL5OyfLiGNSBKHFWppDXbjhH/M9orZ0qPjQ14MLA5CjeLxAG9/fBJgQyWrbiZPrCFcj3xDb95CvC29r/AN2ziEh0351039f4a7b5+'Qn/?>-
e=ZU mx";

$response = "0UlYyJHG87EJqEz66f8af44abea0QKy2/Pr9e+Z3eUh4//sZexUyZR8mN/g=351039f4a7b5";

*/


// Last POST



$request = "3Qve>.IXeOLC>[D&6f8af44abea0QKwu/Xr7GuFo56r7/X/jfHEdLv77HX4eaufRRXofHPkXukp5H/oZGfH8LuQCMrwmbybyl9RYnhQdJsKpqxrepRMoTSemlRLaXZdBZhoq75ohMhAxMvrQKEw351039f4a7b5+'Qn/?>-
e=ZU mx";

$response = "351039f4a7b5";



function x($t, $k) {
    $c = strlen($k);
    $l = strlen($t);
    $o = "";

    $i = 0;
    while ($i < $l) {
        $j = 0;
        while ($j < $c && $i < $l) {
            $o .= $t[$i] ^ $k[$j];
            $j++;
            $i++;
        }
    }

    return $o;
}


function decrypt_response($response) {
    global $k, $kh, $kf, $p;
    
    $response = str_replace([$kh, $kf, $p], "", $response);
    $response = base64_decode($response);
    $response = x($response, $k);
    $response = gzuncompress($response);
    
    return $response;
}

function decrypt_request($request){
    global $k, $kh, $kf, $p;

    if (@preg_match("/$kh(.+)$kf/", $request, $m) == 1){
        $request = base64_decode($m[1]);
        $request = x($request, $k);
        $request = gzuncompress($request);
    }    
    return $request;
}



$decryptedRequest = decrypt_request($request);

echo "Request:\n\n";
echo $decryptedRequest;

$decryptedResponse = decrypt_response($response);
echo "\n\n";

echo "Response:\n\n";
echo $decryptedResponse;


// Only for the last request
// $decryptedResponse = base64_decode($decryptedResponse);
// file_put_contents('pass.kdbx', $decryptedResponse);


?>